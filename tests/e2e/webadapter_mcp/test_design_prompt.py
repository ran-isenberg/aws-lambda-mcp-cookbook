...
async def test_hld_prompt(web_adapter_mcp_url):
    ...
    try:
        ...
        async with Client(web_adapter_mcp_url) as client:
            prompts_list_response = await client.list_prompts()
            ...
            # Call a prompt
            prompt_result = await client.get_prompt('generate_serverless_design_prompt', {'design_requirements': design_requirements})
            # Verify the result
